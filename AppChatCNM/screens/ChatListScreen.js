...
export default function ChatListScreen({ navigation }) {
  ...
  const socket = io("http://172.16.1.126:8004", {
  ...
  const fetchMessagesByConversationId = async (conversationId) => {
    try {
      const response = await fetch(
        `http://172.16.1.126:8004/messages/get/${conversationId}`
      );
  ...
  const handleConversationClick = async (conversation, otherMember) => {
    try {
      ...
      const res1 = await axios.get(
        `http://172.16.1.126:8004/conversations/get/${conversation._id}`
      );
      ...
      if (fullConversation.createGroup?.userId) {
        const res2 = await axios.get(
          `http://172.16.1.126:8004/users/get/${fullConversation.createGroup.userId}`
        );
