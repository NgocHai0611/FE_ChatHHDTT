...
const ContactItem = ({ item, onPress, onUnfriend, isFriend, onAddFriend, onAcceptFriend, onRejectFriend,onCancelFriendRequest, requestInfo }) => (
    <TouchableOpacity
        ...
        <Animated.View entering={FadeInDown.duration(500)}>
            <View style={styles.contactCard}>
                ...
                <View style={styles.contactInfo}>
                    ...
                    {item.phone && <Text style={styles.contactStatus}>Số điện thoại: {item.phone}</Text>}
                </View>
                {isFriend === 'accepted' ? (
                    ...
                    <View style={styles.actionButtonsContainer}>
                        <TouchableOpacity
                            ...
                            onPress={() => onAcceptFriend(requestInfo?._id, requestInfo?.senderId?._id)}
            ...
            if (/^\d+$/.test(query)) {
                try {
                    ...
                    if (result && currentUser && currentUser._id) {
                        ...
                        const isAlreadyFriend = friendList.some(friend => friend._id === result._id);
                        if (isAlreadyFriend) {
                            setFriendStatus('accepted');
                            setIncomingRequest(null);
                        } else {
                            const statusResponse = await checkFriendStatus(currentUser._id, result._id);
                        ...
                        if (statusResponse?.status === 'pending' && statusResponse?.requestId) {
                          // Lưu trữ requestId vào state hoặc searchedUser
                          setSearchedUser(prevUser => ({ ...prevUser, requestId: statusResponse.requestId }));
                      } else {
                          setSearchedUser(prevUser => ({ ...prevUser, requestId: null }));
                      }
                            const incomingRequestFound = incomingRequestsList.find(req => req.senderId?._id === result._id);
    ...
    const getConversation = async (currentUser, contact) => {
        try {
            ...
            const conversation = conversations.find(conv => {
                ...
                return memberIds.includes(currentUser._id) && memberIds.includes(contact._id);
            });
    ...
    const handleContactPress = async (contact) => {
        try {
            ...
            navigation.navigate("ChatScreen", {
                conversation: conversation || await createConversation([currentUser._id, contact._id]),
                  ...
                  onPress: async () => {
                      try {
                          if (currentUser && currentUser._id && friendToRemove._id) {
                              ...
                              setFriendList(prevList => prevList.filter(friend => friend._id !== friendToRemove._id));
                              setFriends(prevList => prevList.filter(friend => friend._id !== friendToRemove._id));
                              ...
                              Alert.alert("Thành công", `Đã hủy kết bạn với ${friendToRemove.username}`);
                          } else {
                              Alert.alert("Lỗi", "Không thể xác định người dùng hoặc bạn bè.");
                          }
                      } catch (error) {
                    ...
                    onPress: async () => {
                        try {
                            if (currentUser && currentUser._id && userToAdd._id) {
                                const response = await addFriend(currentUser._id, userToAdd._id);
                                console.log("Response from addFriend:", response);
                                Alert.alert("Thành công", `Đã gửi lời mời kết bạn đến ${userToAdd.username}`);
    ...
    const handleAcceptFriend = async (requestId, senderId) => {
        try {
            if (requestId && currentUser && currentUser._id && senderId) {
                ...
                Alert.alert("Thành công", `Đã chấp nhận lời mời kết bạn từ ${searchedUser?.username || senderId}`);
                fetchFriendList();
    ...
    const handleRejectFriend = async (requestId) => {
        try {
            if (requestId) {
                ...
                Alert.alert("Thành công", `Đã từ chối lời mời kết bạn từ ${searchedUser?.username}`);
                fetchIncomingRequests(); // Cập nhật lại danh sách yêu cầu
                setSearchedUser(null);
                setFriendStatus(null);
                setIncomingRequest(null);
    ...
    >
        <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.requestItem}>
                            <View style={styles.requestInfo}>
                                <Image source={{ uri: item.senderId?.avatar }} style={styles.requestAvatar} />
                                <Text style={styles.requestName}>{item.senderId?.username}</Text>
                            </View>
                            <View style={styles.requestActions}>
                                <TouchableOpacity
                                    style={[styles.actionButtonModal, styles.acceptButton]}
                                    onPress={() => handleAcceptFriend(item._id, item.senderId?._id)}
                                >
                                    <Text style={styles.actionText}>Chấp nhận</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                ...
                />
                <TouchableOpacity style={styles.closeButton} onPress={closeRequestModal}>
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.searchContainer}>
                <TouchableOpacity style={styles.iconAddFriend} onPress={openRequestModal}>
